...
for (updateLink of updateMediaLinks) {
    updateLink.addEventListener("click", function (e) {
        ...
        if (this.parentElement.children[1].textContent != "") {
            ...
            let postId = this.parentElement.children[2].textContent;
...
const pictureUpdate = function (data, updateLink) {
    if (updateLink.previousElementSibling.textContent == 1) {
        //jumbotron default picture
        updateLink.parentElement.parentElement.parentElement.style[
            "backgroundImage"
...
const videoUpdate = function (data, updateLink) {
    updateLink.parentElement.previousElementSibling.children[0].src =
        data.newVideoUrl;
};
...
let newPictureFieldBtn = document.getElementsByClassName(
